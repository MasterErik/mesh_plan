package com.ezd.mesh.plan.web.rest;
...
import com.ezd.mesh.plan.config.KafkaProperties;
...
class MeshPlanKafkaResourceIT {
    ...
    void setup() {
        ...
        MeshPlanKafkaResource kafkaResource = new MeshPlanKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/mesh-plan-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/mesh-plan-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
